...
export function NotificationProvider({ children }: { children: ReactNode }) {
  ...
  return (
    <NotificationContext.Provider value={value}>
      {children}
      <div className="fixed bottom-4 right-4 left-4 z-50 flex flex-col gap-2 sm:left-auto sm:max-w-sm">
        {notifications.map((item) => (
          <div
            key={item.id}
            className={cn(
              "flex items-center gap-2 rounded-lg border px-4 py-3 shadow-lg backdrop-blur",
              "bg-surface-elevated text-content animate-[toastIn_180ms_ease]",
